...
public class CursoDAO extends Conexao {
    ...
    public void salvarCursoComAluno(CursoModel curso){
        try{
            iniciar();
            getSessao().saveOrUpdate(curso);
            fechar();
            JOptionPane.showInputDialog(null,"Salvo com Sucesso!");
        }catch(Exception e){
            JOptionPane.showInputDialog(null,"Erro ao Salvar: " + e.getMessage());
        }
    }
}